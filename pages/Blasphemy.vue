...
.case-study
    .workwrapper
        .title BLASPHEMY / APPAREL + PACKAGING + CONTENT CREATION
        ...
        .full
            div(v-for="(slide, index) in slideObject.slideCount" @click="changeImg(true)" @mouseenter="showText(false)" @mouseleave="showText(false)")
                ...
                .ctrimg(v-else-if="(index+1 == count)" key=2)
                    video(:src='getImgUrl(index+1, slideObject.ext[index])' autoplay muted loop controls)
  ...
  data () {
    return {
      ...
      slideObject: {
        ...
        ext: ['mp4', 'mp4', 'mp4', 'mp4', 'mp4', 'mp4', 'mp4', 'jpg', 'jpg', 'jpg'],
        icon: ['social', 'social', 'concept', 'web', 'id', 'id', 'id', 'print', 'print', 'print', 'gather', 'id']
      }
    }
  },
  mounted () {
