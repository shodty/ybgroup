<template lang="pug">
vue-draggable-resizable.bs-drag(:w="getW()" :h="getH" :x="getX" :y="getY" @dragging="onDrag" @resizing="onResize" drag-handle=".dragger" :parent="true" :grid="[12,12]"  :style='boxStyles' class-name-active="bs-my-active-class" :active.sync="active")
    .cutoff(@mouseenter='showControls=true' @mouseleave='showControls=false' :style='outlineStyles'  @mousedown="upZ(index)")
        .controller.hide-on-mobile(v-if="resize && active" :style='[active? {"display" : 100 } : {"opacity" : 0} ]')
            .dragger.bs-draggerstyle(:style="draggerStyle()")
            //.bs-inlinediv.strokedwhite.black(@click="changeColor('black')" :style='strokeStyles("black")') ⬤
            //.bs-inlinediv.strokedwhite.blue(@click="changeColor('#008ff8')" :style='strokeStyles("#008ff8")') ⬤
            //.bs-inlinediv.strokedwhite.red(@click="changeColor('#ff5b49')" :style='strokeStyles("#ff5b49")') ⬤
            //.bs-inlinediv.strokedblack.white(@click="changeColor('white')" :style='strokeStyles("white")') ⬤
            .bs-inlinedivtext.strokedwhite(@click="stroked = !stroked" style="color: black" :style="style=strokeStyles('black')")
                img.dragicon(:src="strokeFillIcon()" width="16px")
            .slider
                vue-slider.textsize(v-model="textsize" :min="6" :max="576" :interval="6" :rail-style="{backgroundColor: 'white'}" :process-style='bgc === "#008ff8"? { backgroundColor: "white" } : { backgroundColor: "#008ff8" }' :tooltip-style="{ backgroundColor: 'white', borderColor: '#008ff8', color: '#008ff8' }" )
                    template(v-slot:dot)
                        .circleblack(style="border : 1px solid white !important")
            .slider(style="margin-left: 20px")
                vue-slider(v-model="tracking" :min="0" :max="1" :interval="0.01" :rail-style="{backgroundColor: 'white'}" :process-style='bgc === "#ff5b49"? { backgroundColor: "white" } : { backgroundColor: "#ff5b49" }' :tooltip-style="{ backgroundColor: 'white', borderColor: '#ff5b49', color: '#ff5b49' }")
                    template(v-slot:dot)
                        .circleblack(style="border : 1px solid white !important")
            .bs-inlinedivtext(@click="align = 'left'" :style="bgc === 'black'? 'background: white' : 'background: transparent'")
                img.dragicon(src="../assets/l.png" width="16px")
            .bs-inlinedivtext(@click="align = 'center'" :style="bgc === 'black'? 'background: white' : 'background: transparent'")
                img.dragicon(src="../assets/c.png" width="16px")
            .bs-inlinedivtext(@click="align = 'right'" :style="bgc === 'black'? 'background: white' : 'background: transparent'")
                img.dragicon(src="../assets/r.png" width="16px")
            .inlinedivclose.back.strokedblack(@click="moveToBack" :style="bgc === 'black'? 'color: black' : 'color: white'"  v-tooltip="'send to back'") ▾
            .inlinedivclose.strokedblack(@click="closeBox" :style="bgc === 'black'? 'color: black' : 'color: white'"  v-tooltip="'close'") x
        .controller(v-if="!resize" :style='[ showControls || active? {"opacity" : 100 } : {"opacity" : 0} ]' @mousedown="upZ(index)")
          .dragger.bs-draggerstyle(:style="draggerStyle()")
          .inlinedivclose.back(@click="moveToBack" :style="bgc === 'black'? 'color: black' : 'color: white'"  v-tooltip="'send to back'") ▾
          .inlinedivclose(@click="closeBox" :style="bgc === 'black'? 'color: black' : 'color: white'"   v-tooltip="'close'") x
        .controller.hide-on-desktop(v-if="resize && active" :style='[active? {"display" : 100 } : {"opacity" : 0} ]')
            ...
            .inlinedivclose.back(@click="moveToBack" :style="bgc === 'black'? 'color: black' : 'color: white'" v-tooltip="'send to back'") ▾
            .inlinedivclose(@click="closeBox" :style="bgc === 'black'? 'color: black' : 'color: white'" v-tooltip="'close'") x
        div.bs-textinside(v-if="layout === 'default'" contenteditable="true" spellcheck="false" @click="upZ(index)") {{content}}
        div.bs-textinside.bs-whatcursor(v-if="layout === 'what'" spellcheck="false" @focus="upZ(index)" @click="changeLayout('what')") {{content}}
        div.bs-textinside.bs-whatcursor(v-if="layout === 'why'" spellcheck="false" @focus="upZ(index)" @click="changeLayout('why')") {{content}}
        div.bs-textinside.bs-buycursor(v-if="layout === 'buy'" spellcheck="false" @focus="upZ(index)")
          a(href="https://www.ybgroupshop.us/products/black-supper-typeface" style="color: #008ff8") {{content}}
        div.bs-textinside(v-if="layout === 'video'" @focus="upZ(index)")
        ...
        div.bs-textinsidemobile(v-if="layout === 'mobile'" contenteditable="true" spellcheck="false" @focus="upZ(index)") Type Here
        div.bs-textinside(v-if="layout === 'zine'"  spellcheck="false" @focus="upZ(index)" style="cursor: pointer; line-height: 0")
          img.dragger(src="../assets/bs_gfx.png" width="850px" style="cursor: grab")
</template>
  ...
  data () {
    return {
      ...
      boxText: [
        ...
        'Yellow Brick Group 2021',
        '. \\ -‡- / . \\ . / -‡- \\ .',
        'Spell-Casting',
        '¶',
        '• • •',
        '†††††††††††††',
        'pattern. texture. distort.',
        'Knights',
        '■',
        '‡ ‡ ‡',
        's u p p e r',
        ...
        '•',
        '::::::::',
        'sacred muse. fierce warres. faithful loves'
      ],
      colorPalette: [
        'white',
        'black',
...
.bs-drag
  position: relative
  font-family: 'blacksupper', sans-serif
  font-size: 6em
  line-height : 1
  outline: 0
